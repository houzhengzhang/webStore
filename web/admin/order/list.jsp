                                    ...
                                    <tr onmouseover="this.style.backgroundColor = 'white'"
                                        ...
                                        <td style="CURSOR: hand; HEIGHT: 22px" align="center"
                                            ...
                                            <c:if test="${order.state==2}">
                                                <button><a href="/store/AdminOrderServlet?method=updateOrderByOid&oid=${order.oid}">发货</a></button> </c:if>
                                            <c:if test="${order.state==3}">已发货</c:if>
                                            <c:if test="${order.state==4}">订单完成</c:if>
                                        </td>
                                        <td align="center" style="HEIGHT: 22px" width="60%">
                                            <input type="button" value="订单详情" id="${order.oid}" class="myClass"/>
                                            <table border="1" width="100%" style="display: none" >
                                                    <%--<tr><th>商品</th><th>名称</th><th>单价</th><th>数量</th></tr>--%>
                                                    <%--<tr><td><img src="/store/products/1/c_0001.jpg" width="50px"></td><td></td>--%>
                                                    <%--<td></td><td></td></tr>--%>
                                            </table>
                                        </td>
                                    </tr>
                ...
                if (txt == "订单详情") {
                    $.post(url, obj, function (data) {
                        ...
                        $.each(data, function (i, obj) {
                            var td = "<tr><td><img src='/store/" + obj.product.pimage + "' width='50px' ></td><td>" +
                                obj.product.pname + "</td><td>" + obj.product.shop_price + "</td><td>"
                                + obj.quantity + "</td></tr>";
                            $tb.append(td);
                        });
                    }, "json");
